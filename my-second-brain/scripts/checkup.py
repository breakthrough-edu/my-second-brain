...
# --------------------------------------------------------------------------
# Nothing in this script reads `doctrine_version:`, and there is no
# house-vintage check in this product. Adding one back here is not a fix.
#
# Scaffold still WRITES `doctrine_version:` into both of a vault's records on
# every new house (references/scaffold-spec.md says why): it records which
# generation of the product built that house, and a house that goes unstamped
# can never be dated afterwards, because the folders do not say it. So the key
# is not an orphan and must not be cleaned up as one.
# --------------------------------------------------------------------------
...
#
# The retired vintage check was the other member until 2026-08-20 and left
# with the check itself. ⛔ The set was not deleted along with it: it was never
# that check's private switch, it is the general "do not trust the count above"
# one, and `schema-unreadable` still earns it (a section 8 that will not parse
# means the schema checks below did not run at all, so a clean report proves
# nothing). A set of one is the honest state, not a leftover.
INVALIDATING_CHECKS = {"schema-unreadable"}
...
def print_report(vault, cfg, source, findings, notes):
    ...
    #
    # ⛔ The banner keys on INVALIDATING_CHECKS, never on a check name written
    # here. It was written for the vintage error that has since been retired,
    # and if that name had been spelled out in this function the banner would
    # have gone silent the day that check left, taking `schema-unreadable` down
    # with it. It says what was OBSERVED and stops there: whichever check did
    # not clear, the only claim made is that the report above may be describing
    # something it did not actually test.
