...
- **`method-builder`** writes one Method when a piece of WORK closes: how the owner did that kind of thing, in their words, and composes several of them into a playbook when the owner asks. ⛔ It matters that this one ships for two reasons. It is the other half of the closeout pair, and the two never fire at the same moment (one closes a session, one closes a job). And it is the **only writer of a playbook folder's door**, which doctrine §9.1 makes mandatory from the day such a folder exists: ship the law without this skill and the first method written creates a folder that is out of shape the moment it is made.
