...
1. **Doors and the directory, four things at once.** (a) **Exists:** every room, lane, brand subfolder, wing **and playbook folder** has a door. ⭐ **The playbook folders are the ones worth naming here**, because they are the only doors nothing else audits: they are made at runtime by `breakthrough-method-builder`, so the install-time wiring checks never see them, and doctrine §9.1 makes the door mandatory from the day the folder exists. A playbook folder without a door is a finding. (b) **Unique:** exactly one `_`-prefixed file per folder, never two (§5). (c) **Content:** the door's text still describes what is actually in the folder. (d) **Directory:** `Home.md` audited against the filesystem **in both directions**, every folder listed exists and every folder that exists is listed, minus the three documented exemptions (inside a project folder, inside `99_Meta/Skills/`, and `99_Meta/memory-archive/` itself). ⭐ **A folder that turns out to be a whole new wing is not a directory fix.** Home, the doors, `CLAUDE.md` and §8 all have to move together, which is `breakthrough-vault-guardian`'s job; name it here and propose opening it, ⛔ never paper over it with a Home line.

    ⭐ **(c) is the one with teeth.** A door that exists and is unique can still be a lie: it was written the week the room was moved into and the room has had a year since. Scan door **content** for **rooms, brand folders and playbook folders**, around seventeen files plus however many kinds of work the owner has written up. ⭐ **On a playbook door, (c) means the top half only.** The bottom half is a record of things that already happened and is never edited to stay correct; ⛔ do not rewrite rows, and ⛔ do not fill in a missing result from memory, which is what beat 1 of a real session does with the source in front of it (§9.3). ⛔ **Do not scan `## Observations` sections**, ⛔ do not scan the SOP menu (`breakthrough-sop-builder` reconciles that itself every time it runs), and ⛔ do not judge a wing guide's `## Current state` here, which is prose and gets its own check below.
...
4b. **Links and dependency trees.** No symlink inside the vault points out of it, and no `node_modules` or other dependency tree has landed inside it. ⭐ **This is the one check whose damage is invisible from inside the vault**: Obsidian indexes whatever it can reach and follows symlinks, so one link can upload an entire external tree as vault content while the folder on disk still looks small. The numbers then disagree with each other and the cause gets blamed on something else first (version history is the usual wrong answer). A link pointing back INTO the vault reports at INFO and is not damage, only duplication in search results. ⛔ To remove one, plain `rm`, never `rm -r`: the recursive form passes through the link and deletes what it points at.
