...
⚠️ **First, check it is yours to take.** **"If the material carries links or a structure the owner wants kept, or it cannot be mapped and moved in one sitting, it is a migration and `breakthrough-vault-migrator` runs it; otherwise it is a bulk move-in and capture handles it tonight."** ⛔ That sentence is written word for word in `breakthrough-vault-migrator`'s own `SKILL.md`; an edit to one is an edit to both. What this stage keeps is the pile that has no links and no old structure worth preserving and fits in tonight; what it hands over is the house move. ⛔ Do not start a house move here and finish it never: a bulk move-in that stalls half done leaves material stranded between two homes, which is the one outcome neither skill can clean up afterwards.
