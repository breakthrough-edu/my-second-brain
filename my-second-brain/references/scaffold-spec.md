...
8. `99_Meta` state files all exist, and `maintenance-state.md` carries `cadence_days`. **And the house says which generation built it, in both places, with the same number:** `99_Meta/structure-doctrine.md` carries `doctrine_version:` in its frontmatter (it comes from the template, so this is really a check that the copy did not drop it) and `bootstrap-progress.md` carries the same integer. A vault that leaves setup with neither is indistinguishable, forever after, from a vault built by the previous generation, and `SKILL.md`'s house-vintage gate would then fall through to reading folder shapes on a house that is perfectly current.
