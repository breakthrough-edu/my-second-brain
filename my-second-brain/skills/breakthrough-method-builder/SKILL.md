...
3. `04_Methodology/Playbooks/`: the folder names first, then the door of any folder that could plausibly hold this work, then that folder's methods. ⛔ Not a flat glob over the layer: it is folders now (doctrine §9.1), and a flat pattern for `*-method.md` at the top level matches nothing. ⭐ **This is the step that pays for itself:** if a method for this work already exists, ⛔ you are not writing a new one, and the folder list is also what you put in front of the owner in the one question above.
...
3. **Write the playbook** from the Playbook template into that same folder, `<the-kind-of-work>-playbook.md`. `references:` is filled at birth with the lessons and decisions it leans on, ⛔ once, never revisited (doctrine §7).
4. **Flip each contributing method** to `status: superseded` with `distilled_into:` naming the playbook. ⛔ Never delete one: which fights this came out of has to stay answerable.
