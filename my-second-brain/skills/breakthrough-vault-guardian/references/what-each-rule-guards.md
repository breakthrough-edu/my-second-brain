...
| `started` `due` | The project cannot be drawn on the timeline. With neither, the swimlane leaves it off entirely; with `due` alone it becomes a milestone diamond. | `scripts/deck.py` → `"started": str(fm.get("started", "")).strip() or None` reads them, and `cmd_doctor` reports the gap → `if not p["due"] and not p["started"]` · the timeline and countdown in `scripts/deck-template.html`. | Already optional. |
