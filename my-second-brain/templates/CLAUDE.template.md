...
- **An operational "how do we do X" question**: find the SOP in `03_SOP/`, answer FROM it, and update it in the same move if answering revealed it is stale. When the answer turns on a judgment rather than a step, open the playbook the SOP's `playbook:` key names, and the lane's active decisions with it; an SOP without that key has no playbook behind it, which is legal. No SOP yet? Say so and offer to write one. ⛔ Do not improvise a finished-looking SOP from one answer.
