...
8. **Maintenance doorbell:** read `{{VAULT_PATH}}/99_Meta/maintenance-state.md`. Compare `last_tidy` **and** `last_distill` against `cadence_days` from that same file (never a hardcoded 7: {{YOUR_NAME}} can change the rhythm and this must follow). If either is overdue, mention it once in the morning brief (the dates are seeded at setup, so day one never fires; a missing file or empty date means maintenance is due).
   ...
   - `last_tidy` overdue → "Your vault has not had its upkeep pass in N days. Say the word and I will run second-brain maintenance."
   - `last_distill` overdue → "It has been N days since anything got distilled. Say the word and I will run the second-brain distill."
   ...
   ⭐ **Why this is spelled out rather than left to phrasing:** this doorbell fires on `either is overdue` and used to answer, every time, that it would run the *distill*. Once the ritual became two files, that sentence was pointing at the wrong one whenever it was the upkeep pass that had lapsed. It is the first thing this product says to {{YOUR_NAME}} in a week, so it is not a caption, it is the product speaking.
   ...
   ⛔ **There is no harvest doorbell any more and it must not be reinstated.** Until 2026-08-20 this step also ran a weekly pass over new Claude Code sessions, unasked, and proposed lines to keep. That pass is retired: what a session was worth is now decided at that session's own closeout, by whoever was in it. `session-history` is still installed and still searchable on demand; ⛔ it rings no bell.
...
9. **Dashboard doorbell (one line, and it never changes).** Run the `deck.py build` that ships in the `my-second-brain` skill's payload against `{{VAULT_PATH}}`, fail-soft. ⭐ **This is the only rebuild there is.** Weekly maintenance used to run a second one as a backstop for weeks nobody opened a session, and that backstop was never real: it was reached through a doorbell living in this very session start. One rebuild that runs whenever {{YOUR_NAME}} shows up beats two that share a single point of failure. The engine, the display template and every rule about what it draws live over there and are updated by `npx skills update`; this skill only presses the button. If it errors or `python3` is missing, say nothing and carry on: a dashboard must never delay a morning brief.
...
| "let's talk about how I've grown" / "how has this year gone" | Read the review layer: `02_Command-Base/Reviews/` (the weekly reviews, and the monthly themes, which are filed as a chain, each one linked to the theme it replaced and to the theme that replaced it, so they read as a line rather than a pile) together with `02_Command-Base/Decisions/`. Mirror the growth back, in {{YOUR_NAME}}'s own words wherever the record carries them. ⭐ **This is where the monthly retro went.** It does not ship as a filed note, it ships as this conversation: what a retro was for is already sitting in the layer, and a report nobody rereads is just a fourth thing to write. ⛔ **Do not give it a skill of its own:** the layer already exists, so the capability costs nothing to have. ⛔ **Do not narrate more than the record supports:** when a source is empty, name it and say what empty means there, which is usually that those weeks went unwritten rather than that nothing happened |
